...
5.  Power on your ESP32, it will present you with an AP named ```SmartA-abc``` (while ```SmartA``` can be changed in the portal and ```abc``` is a unique id for each esp32)
...
6.  The data is published to the MQTT Topic ```SmartA/{hostname}``` while the hostname is the one which you can define in Settings page of the captive portal.
